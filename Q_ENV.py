...
class QUADROTOR(gym.Env):
    ...
    def __init__(self):
        ...
        high_s = np.array([
            ...
            q_bound,
            r_bound,])
        high_a = np.array([0.02,0.03,0.003,0.05,0.02,0.02])
    ...
    def ode_step(self, F,M):
        ...
        self.state=s_[1]
               ...
               or abs(self.state[1]) > 1 \
               or abs(self.state[2]) > 2 \
               or self.state[2] < 0

        done = bool(done)

        r1=2*(0.5-abs(self.state[0]))
        r2=2*(0.5-abs(self.state[1]))
        r3=1-abs((self.state[2]-1))
        r = np.sign(r1) * ((10 * r1) ** 2)+ np.sign(r2) * ((10 * r2) ** 2)+ 5*np.sign(r3) * ((10 * r3) ** 2)
    ...
    def policy_step(self,desire_state):
        hit=0
    ...
    def reset(self):
        ...
        # self.state[0] = np.random.uniform(low=-0.25, high=0.25)
        # self.state[1] = np.random.uniform(low=-0.25, high=0.25)
