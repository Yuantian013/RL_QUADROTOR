...
class DDPG(object):
    ...
    def save_result(self):
        save_path = self.saver.save(self.sess, "Model/V1.ckpt")
...
for i in range(MAX_EPISODES):
    ...
    for j in range(MAX_EP_STEPS):
        ...
        if j == MAX_EP_STEPS - 1:
            ...
            print('LR',LR_A,'Episode:', i, 'step',j,' Reward: %i' % int(ep_reward),"EWMA_step = ",EWMA_step[0,i+1],"EWMA_reward = ",EWMA_reward[0,i+1],s_[0],s_[1],s_[2],'VAR',var,(time.time() - t1))
            ...
            EWMA_reward[0, i + 1] = EWMA_p * EWMA_reward[0, i] + (1 - EWMA_p) * ep_reward
            if hit==1:
                print('LR',LR_A,'Crush,','Episode:', i, 'step',j,' Reward: %i' % int(ep_reward),"EWMA_step = ",EWMA_step[0,i+1], "EWMA_reward = ", EWMA_reward[0, i + 1],s_[0],s_[1],s_[2],'VAR',var,(time.time() - t1))
            else :
                print('LR',LR_A,'Hit walls,', 'Episode:', i, 'step', j, ' Reward: %i' % int(ep_reward), "EWMA_step = ",
                      EWMA_step[0, i + 1], "EWMA_reward = ", EWMA_reward[0, i + 1], s_[0], s_[1], s_[2],'VAR',var,
                      (time.time() - t1))
            break
    if ep_reward > 2000000:
        fig = plt.figure()
        ax = plt.subplot(projection='3d')  # 创建一个三维的绘图工程
